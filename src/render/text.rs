...
impl TextRenderer {
    ...
    pub fn get_coordinate_range(&self) -> ((f32, f32), (f32, f32)) {
        (
            (
                self.viewport.logical_bounds.0,
                self.viewport.logical_bounds.1,
            ),
            (
                self.viewport.logical_bounds.2,
                self.viewport.logical_bounds.3,
            ),
        )
    }
    ...
    pub fn initialize(&mut self) -> Result<(), String> {
        ...
        self.gl
            .set_blend_func(gl::SRC_ALPHA, gl::ONE_MINUS_SRC_ALPHA)?;
    ...
    pub fn load_font(&mut self, name: &str, font_path: &str, size: u32) -> Result<(), String> {
        ...
        // Get font metrics
        let metrics = font_info
            .fontdue_font
            .as_ref()
            .unwrap()
            .metrics('A', size as f32);
    ...
    fn render_glyph(
        ...
        self.gl
            .set_uniform_2f(pos_loc, gl_position.x, gl_position.y)?;
