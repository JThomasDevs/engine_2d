...
impl WindowManager {
    ...
    fn detect_monitors(glfw: &mut Glfw) -> Vec<MonitorInfo> {
        ...
        glfw.with_primary_monitor(|_, monitor| {
            if let Some(monitor) = monitor {
                let video_mode = monitor.get_video_mode().unwrap_or_else(|| glfw::VidMode {
                    width: 1920,
                    height: 1080,
                    red_bits: 8,
                    green_bits: 8,
        ...
        for monitor in &monitors {
            println!(
                "  {}: {}x{}@{}Hz",
                monitor.name, monitor.resolution.0, monitor.resolution.1, monitor.refresh_rate
            );
        }
    ...
    pub fn toggle_fullscreen(&mut self) -> Result<(), String> {
        ...
        match self.current_mode {
            DisplayMode::Windowed => {
                ...
                println!(
                    "Saving windowed position: ({}, {}) size: {}x{}",
            ...
            DisplayMode::BorderlessFullscreen | DisplayMode::ExclusiveFullscreen => {
                ...
                self.glfw.with_primary_monitor(|_, monitor| {
                    if let Some(monitor) = monitor {
                        ...
                        let video_mode =
                            monitor.get_video_mode().unwrap_or_else(|| glfw::VidMode {
                                width: 1920,
                                height: 1080,
                                red_bits: 8,
                                green_bits: 8,
    ...
    pub fn set_minimum_size(&mut self, width: u32, height: u32) {
        self.window
            .set_size_limits(Some(width), Some(height), None, None);
    }

    /// Set maximum window size
    pub fn set_maximum_size(&mut self, width: u32, height: u32) {
        self.window
            .set_size_limits(None, None, Some(width), Some(height));
    }
