...
fn test_action_categories() {
    ...
    let combat_action = GameAction {
        ...
        input_type: InputType::Digital,
        default_bindings: vec![InputBinding::Single(PhysicalInput::Mouse(
            MouseButton::Left,
        ))],
        metadata: ActionMetadata::default(),
    };
...
fn test_context_system() {
    ...
    let context =
        InputContext::new("test_context".to_string(), 1).disable_action("TEST_ACTION".to_string());
...
fn test_analog_input_values() {
    ...
    let action = GameAction {
        ...
        default_bindings: vec![InputBinding::Analog {
