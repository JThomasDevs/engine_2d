...
fn main() {
    ...
    println!("Distance between v1 and v2: {:.2}", vector::distance(v1, v2));
    ...
    println!("Linear interpolation: {:.2}", interpolation::lerp(start, end, t));
    ...
    println!("\n🛠️ Utility Functions:");
    let value = 150.0;
    println!("Clamp 150 to [0, 100]: {:.2}", utils::clamp(value, 0.0, 100.0));
    println!("Map 50 from [0, 100] to [0, 1]: {:.2}", utils::map(50.0, 0.0, 100.0, 0.0, 1.0));
    println!("45° to radians: {:.2}", utils::deg_to_rad(45.0));
    println!("π/2 radians to degrees: {:.2}", utils::rad_to_deg(std::f32::consts::PI / 2.0));
    ...
    println!("Point {:?} in rectangle: {}", point, rect.contains_point(point));
    println!("Point {:?} in circle: {}", point, circle.contains_point(point));
    println!("Rectangle intersects circle: {}", circle.intersects_rect(&rect));
