...
impl PendulumAnimation {
    ...
    fn initialize_textures(&mut self, sprite_renderer: &mut SpriteRenderer) -> Result<(), String> {
        ...
        self.red_texture = Some(sprite_renderer.texture_manager().create_color_texture(
            64,
            64,
            (255, 0, 0, 255),
        )?);
        self.green_texture = Some(sprite_renderer.texture_manager().create_color_texture(
            64,
            64,
            (0, 255, 0, 255),
        )?);
        self.blue_texture = Some(sprite_renderer.texture_manager().create_color_texture(
            64,
            64,
            (0, 0, 255, 255),
        )?);
        self.yellow_texture = Some(sprite_renderer.texture_manager().create_color_texture(
            64,
            64,
            (255, 255, 0, 255),
        )?);
