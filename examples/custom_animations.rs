...
impl CircularAnimation {
    ...
    fn initialize(&mut self, sprite_renderer: &mut SpriteRenderer) -> Result<(), String> {
        if self.red_texture_id.is_none() {
            self.red_texture_id = Some(sprite_renderer.texture_manager().create_color_texture(
                64,
                64,
                (255, 0, 0, 255),
            )?);
...
impl BouncingAnimation {
    ...
    fn initialize(&mut self, sprite_renderer: &mut SpriteRenderer) -> Result<(), String> {
        if self.green_texture_id.is_none() {
            self.green_texture_id = Some(sprite_renderer.texture_manager().create_color_texture(
                64,
                64,
                (0, 255, 0, 255),
            )?);
...
impl SpinningAnimation {
    ...
    fn initialize(&mut self, sprite_renderer: &mut SpriteRenderer) -> Result<(), String> {
        if self.blue_texture_id.is_none() {
            self.blue_texture_id = Some(sprite_renderer.texture_manager().create_color_texture(
                64,
                64,
                (0, 0, 255, 255),
            )?);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let animation_choice = std::env::args()
        .nth(1)
